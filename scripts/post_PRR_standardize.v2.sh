...
with out.open("w", newline="") as fh:
    ...
    w.writerow([
        ...
        "has_PF00072", "has_PF06203", "domain_class", "min_ali_from", "max_ali_to"
    ])
    for pid in excluded_ids:
        top1 = top1_map.get(pid, "NA")
        length = len_map.get(pid, "NA")
        info = domain_map.get(pid, {})
        has1 = info.get("has_PF00072", "NA")
        has2 = info.get("has_PF06203", "NA")
...
{
  echo "PRR QC (standardized v2)"
  echo "Candidates (PF00072 + PF06203): $(wc -l < "${D}/${F}_candidates.sorted.id")"
