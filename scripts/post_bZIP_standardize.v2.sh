...
with out.open("w", newline="") as fh:
    ...
    w.writerow([
        ...
        "has_PF00170", "has_PF07716", "domain_class", "min_ali_from", "max_ali_to"
    ])
    for pid in excluded_ids:
        top1 = top1_map.get(pid, "NA")
        length = len_map.get(pid, "NA")
        info = domain_map.get(pid, {})
        has1 = info.get("has_pf00170", "NA")
        has2 = info.get("has_pf07716", "NA")
        ...
        if pid in nohit:
            ...
            reason = "hard_remove_non_bzip"
        elif pid in amb:
            reason = "ambiguous_top1"

        w.writerow([pid, reason, top1, length, has1, has2, cls, min_from, max_to])
PY

{
  echo "bZIP QC (standardized v2)"
  echo "Candidates (PF00170 or PF07716): $(wc -l < "${D}/${F}_candidates.sorted.id")"
