...
for f in "$PROTEOME" "$CAND_ID" "$HIGH_ID" "$RESC_ID" "$LEN_TSV" "$PFAM"; do
...
# ---- IMPORTANT: never sort back into the same file directly ----
...
{
  echo "PRK QC (standardized v2)"
  echo "Candidates (PF00485): $(wc -l < "${D}/${F}_candidates.sorted.id")"
